...
class ButtonsManager(object):
    ...
    def get_start_button(self):
        return [
            [self.button.text("🔴⚪⚫RafAutoBot🔴⚪⚫", resize=True)]
        ]
    ...
    def get_menu_buttons(self):
        return [
            [self.button.text("🚀 Iniciar", resize=True)],
            [self.button.text("⏹ Parar", resize=True)],
            [self.button.text("⚙️ Configurar", resize=True),
             self.button.text("🆘 Menu Inicial", resize=True)],
        ]

    def get_account_buttons(self, data):
        return [
            [self.button.inline(f"Usuário = {data['user']['email']}", "username")],
            [self.button.inline(f"Senha = {data['user']['password']}", "password")],
            [self.button.inline(f"✅", "confirm"), self.button.inline(f"➡", "more")]
        ]
    ...
    def get_more_buttons(self, data):
        return [
            ...
            [self.button.inline(f"Gerenciamento Milion = {data['settings']['white_gerenciamento_tk']}", "white_gerenciamento_tk")],
